...
# Code: Step 4 - simulation - quadratic dr relation, penalized epi model      #
# Machine: Cannon                                                             #
###############################################################################
...
dir_results <- '/n/home07/yaw624/EME/results/Quadratic_Penalized/'   ### change directory for saving results



######################################### 1. function ################################################
simulate_results <- function(key,covar,b0,b1,b2,n.reps){
  ...
  results <- data.frame(matrix(NA, nrow=n.reps*12, ncol=204))
  names(results) <- c('type','corr_magnitude','pollutants',seq(0, 20, by=0.1))
  results$b0 <- b0
  results$b1 <- b1
  results$b2 <- b2
  ...
  for (i in 1:n.reps){
    ...
    newdata_1 <- data.frame(pm25_star=seq(0,20,by=0.1))
    newdata_2 <- data.frame(pm25.x=seq(0,20,by=0.1))
    newdata_3 <- data.frame(matrix(0, nrow=length(seq(0,20,by=0.1)), ncol=42))
    ...
    results[((i-1)*12+1),4:204] <- as.data.frame(predict(mod_classical_multi_ind_error_sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25_star.
    DATA$pm25_star <- NULL
    ...
    results[((i-1)*12+2),4:204] <- as.data.frame(predict(mod_classical_multi_ind_error_2sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25_star.
    DATA$pm25_star <- NULL
    ...
    results[((i-1)*12+3),4:204] <- as.data.frame(predict(mod_classical_multi_ind_error_3sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25_star.
    DATA$pm25_star <- NULL
    ...
    results[((i-1)*12+4),4:204] <- as.data.frame(predict(mod_classical_multi_sp_error_sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25_star.
    DATA$pm25_star <- NULL
    ...
    results[((i-1)*12+5),4:204] <- as.data.frame(predict(mod_classical_multi_sp_error_2sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25_star.
    DATA$pm25_star <- NULL
    ...
    results[((i-1)*12+6),4:204] <- as.data.frame(predict(mod_classical_multi_sp_error_3sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25_star.
    DATA$pm25_star <- NULL
    ...
    results[((i-1)*12+7),4:204] <- as.data.frame(predict(mod_Berkson_multi_ind_error_sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25.x.
    ...
    results[((i-1)*12+8),4:204] <- as.data.frame(predict(mod_Berkson_multi_ind_error_2sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25.x.
    ...
    results[((i-1)*12+9),4:204] <- as.data.frame(predict(mod_Berkson_multi_ind_error_3sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25.x.
    ...
    results[((i-1)*12+10),4:204] <- as.data.frame(predict(mod_Berkson_multi_sp_error_sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25.x.
    ...
    results[((i-1)*12+11),4:204] <- as.data.frame(predict(mod_Berkson_multi_sp_error_2sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25.x.
    ...
    results[((i-1)*12+12),4:204] <- as.data.frame(predict(mod_Berkson_multi_sp_error_3sd, newdata = newdata,type="terms", se.fit = TRUE))$fit.s.pm25.x.
    ...
    for (b1 in b1_list) {
      for (b2 in b2_list) {
        if (!file.exists(paste0(dir_results,'results_set',i,'_b0',b0,'_b1',b1,'_b2',b2,'.rds'))){
          results_temp <- simulate_results(key,covar,b0,b1,b2,n.reps)
