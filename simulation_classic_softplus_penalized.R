#########################################################################################################
# Project: EME                                                                                          #
# Code: simulate mortality with softplus model and test with penalized splines - classical error        #
# Author: Yaguang Wei                                                                                   #
#########################################################################################################
...
dir_results <- '/n/home07/yaw624/EME/results/classical/softplus_penalized/'   ### change directory
...
simulate_results<-function(key,covar,k,b,n.reps,n.obs){
  ...
  for (i in 1:n.reps){
    ...
    y<-rpois(n.obs,exp(log(1+exp(k*(exposure-b)))+4.795e-04*ozone_summer+7.526e-04*no2+6.693e-04*temp+7.395e-04*rh
    ...
    # y_plot=log(1+exp(0.3*(x-26)))
    # plot(x_plot,y_plot-y_plot[1],ylim=c(-0.1,0.2),type = "l")
    # lines(x_plot, predict_try1_bam$fit.s.exposure1_star.-predict_try1_bam$fit.s.exposure1_star.[1], type = "l", col = "orange")
...
# end.time <- Sys.time()
# end.time - start.time
# # # Time difference of 1.275107 hours
