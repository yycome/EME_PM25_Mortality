#################################################################################################
# Project: EME                                                                                  #
# Code: simulate mortality with quadratic PM and test with penalized splines - Berkson error    #
# Author: Yaguang Wei                                                                           #
#################################################################################################
...
dir_results <- '/media/qnap4/Yaguang/EME/results/Berkson/quadratic_penalized/'
...
simulate_results<-function(key,covar,b0,b1,b2,n.reps,n.obs){
  ...
  results <- data.frame(matrix(NA, nrow=n.reps*6, ncol=201))
  names(results) <- seq(0, 20, by=0.1)
  results$b0 <- NA
  results$b1 <- NA
  results$b2 <- NA
  results$exp <- NA
  ...
  for (i in 1:n.reps){
    ...
    newdata_1 <- data.frame(exposure=seq(0,20,by=0.1))
    ...
    #extract results
    results[((i-1)*6+1),(1:201)] <- predict_try1_bam$fit.s.exposure
    results[((i-1)*6+2),(1:201)] <- predict_try2_bam$fit.s.exposure2_star.
    results[((i-1)*6+3),(1:201)] <- predict_try3_bam$fit.s.exposure3_star.
    results[((i-1)*6+4),(1:201)] <- predict_try4_bam$fit.s.exposure4_star.
    results[((i-1)*6+5),(1:201)] <- predict_try5_bam$fit.s.exposure5_star.
    results[((i-1)*6+6),(1:201)] <- predict_try6_bam$fit.s.exposure6_star.
    results[(((i-1)*6+1):((i-1)*6+6)),'b0'] <- b0
    results[(((i-1)*6+1):((i-1)*6+6)),'b1'] <- b1
    results[(((i-1)*6+1):((i-1)*6+6)),'b2'] <- b2
...
# end.time <- Sys.time()
# end.time - start.time
# # Time difference of 2.502743 hours
...
b0_list <- c(log(8),log(12),log(20))
b0 <- b0_list[1]      ### change b0 combinationas to run
b1_list <- c(0.012,0.019)
b2_list <- c(-0.0003,-0.0002,-0.0001,0.0001,0.0002,0.0003)
