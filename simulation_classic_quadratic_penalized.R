#################################################################################################
# Project: EME                                                                                  #
# Code: simulate mortality with quadratic PM and test with penalized splines - classical error  #
# Author: Yaguang Wei                                                                           #
#################################################################################################
...
dir_results <- '/media/qnap4/Yaguang/EME/results/quadratic_penalized/'
...
simulate_results<-function(key,covar,b0,b1,b2,n.reps,n.obs){
  ...
  for (i in 1:n.reps){
    ...
    newdata_try1_1 <- data.frame(exposure1_star=seq(0,20,by=0.1))
    newdata_try1_2 <- data.frame(matrix(0, nrow=length(seq(0,20,by=0.1)), ncol=42))
    ...
    #extract results
    results[((i-1)*6+1),(1:201)] <- predict_try1_bam$fit.s.exposure1_star.
